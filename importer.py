...
class imageCutter:
	def __init__(self, base_path: str, sheet: str | pygame.surface.Surface) -> None:
	...
	def cutImages(self, width: int, height: int, frame: int) -> pg.surface.Surface:
    		''' cut images from a sprite sheet horizontally in squares '''
		surface = pygame.Surface((width, height), pygame.SRCALPHA)
		surface.blit(self.sheet, (0, 0), (width * frame, 0, width, height))
		return surface

	def cutTilesUndefined(self, width: int, height: int, start_hor: int, start_ver: int) -> pg.surface.Surface:
    		''' cut image from a spritesheet from any part of the sheet '''
		surface = pygame.Surface((width, height), pygame.SRCALPHA)
		surface.blit(self.sheet, (0, 0), (start_hor, start_ver, width, height))
		return surface

	def cutRandomTiles(self, width: int, height: int, start_hor: int, start_ver: int, scale: tuple = None) -> pg.surface.Surface:
